  ...
  useEffect(() => {
    ...
    const fetchTaskDetails = async () => {
      try {
        ...
        if (taskData.responsibilities) {
          setResponsibilities(taskData.responsibilities);
        }
        ...
        if (taskData.assignTo && taskData.assignTo.length > 0) {
          ...
          if (isTeamTask) {
            // Initialize employee progress from task data
            const progressMap = {};
            if (taskData.userStatuses) {
              Object.keys(taskData.userStatuses).forEach(empId => {
                progressMap[empId] = statusMap[taskData.userStatuses[empId]] || "Not Started";
              });
            } else {
              // Fallback if userStatuses not available
        ...
        {task?.assignTo?.map((employeeId, index) => {
          ...
          return (
            <tr key={index}>
              ...
              <td>{responsibilities[employeeId] || "Not specified"}</td>
              <td>
                <select
                  ...
                  className={`status-select ${(employeeProgress[employeeId] || "")
                    .toLowerCase()
                    .replace(" ", "-")}`}
                     style={{marginLeft:'20px', padding:'5px 10px', borderRadius:'7px'}}
                >
              ...
              <div className="task-info">
                ...
                {!location.state?.isTeamTask && (
                  <div className="progress-section">
                    <label style={{marginBottom:'10px'}}><strong>Progress:</strong></label>
                    <select
                      value={progress}
